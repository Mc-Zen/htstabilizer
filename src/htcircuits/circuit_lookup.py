...
one class. 
  ...
  [graph-id]:[circuit specification]

I.e. they contain the graph in compressed form as a bitstring integer
...
def circuit_lookup(num_qubits: int, connectivity: str, id):
    ...
    return parse_circuit(num_qubits, lines[id].split(":")[1])

def parse_circuit(num_qubits: int, circuit_string: str):
